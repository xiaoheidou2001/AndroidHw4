...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float hourX = (float) Math.cos(Math.toRadians((hour + 1.0*minute / 60 + 1.0*second / 3600)*30+270))*radius*HOUR_NEEDLE_LENGTH_RATIO;
        float hourY = (float) Math.sin(Math.toRadians((hour + 1.0*minute / 60 + 1.0*second / 3600)*30+270))*radius*HOUR_NEEDLE_LENGTH_RATIO;
        float minuteX = (float) Math.cos(Math.toRadians((minute + 1.0*second / 60)*6+270))*radius*MINUTE_NEEDLE_LENGTH_RATIO;
        float minuteY = (float) Math.sin(Math.toRadians((minute + 1.0*second / 60)*6+270))*radius*MINUTE_NEEDLE_LENGTH_RATIO;
        float secondX = (float) Math.cos(Math.toRadians(second*6+270))*radius*SECOND_NEEDLE_LENGTH_RATIO;
        float secondY = (float) Math.sin(Math.toRadians(second*6+270))*radius*SECOND_NEEDLE_LENGTH_RATIO;
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        needlePaint.setColor(Color.WHITE);
        canvas.drawLine(centerX,centerY,hourX + centerX,hourY + centerY,needlePaint);
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        needlePaint.setColor(Color.WHITE);
        canvas.drawLine(centerX,centerY,minuteX + centerX,minuteY + centerY,needlePaint);
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        needlePaint.setColor(Color.WHITE);
        canvas.drawLine(centerX,centerY,secondX + centerX,secondY + centerY,needlePaint);
        //画中心点
        needlePaint.setStrokeWidth(20);needlePaint.setStyle(Paint.Style.FILL_AND_STROKE);
        needlePaint.setColor(Color.WHITE);
        canvas.drawCircle(centerX,centerY,30,needlePaint);
        needlePaint.setStrokeWidth(20);needlePaint.setStyle(Paint.Style.FILL_AND_STROKE);
        needlePaint.setColor(Color.LTGRAY);
        canvas.drawCircle(centerX,centerY,15,needlePaint);
        //画数字
        needlePaint.setColor(Color.WHITE);needlePaint.setTextSize(50);
        needlePaint.setStyle(Paint.Style.FILL);needlePaint.setTextAlign(Paint.Align.CENTER);
        String curr_time = String.format("%02d:%02d:%02d", hour, minute, second);
        canvas.drawText(curr_time,centerX,centerY - 100,needlePaint);
        /**
         * 思路：
